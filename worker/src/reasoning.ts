...
3. 直线过已知点 A 与二次曲线交于 B,C 时，优先用 P(t)=A+t(1,k) 参数化；先用根的乘积判定 B,C 在 A 的同侧或异侧，再处理距离和无向面积的绝对值。
4. 若两个三角形共用一条所在直线上的底，优先写成“公共底长 × 到该直线的距离”；不得在未判定符号时将两个无向面积之差合并为行列式之差。
...
4. 对圆锥曲线和解析几何，重点检查交点方程、韦达关系、根的符号、辅助点存在性、无向面积的绝对值和候选解代回。
